        ...
        try:
            with httpx.Client(timeout=30.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"AgentRouter API returned status {response.status_code}: {response.text}")
...
"""
        ...
        return (
            f"**[IntelPulse Analyst Response]**\n\n"
            f"Based on the scanned data for your query (*'{user_question}'*):\n"
            f"- **Research Context:** Analyzed {len(context_research)} scientific publications emphasizing agentic reliability and tool orchestration.\n"
            f"- **Competitor Signal:** Tracked {len(context_competitors)} market releases highlighting rapid feature rollouts and pricing shifts.\n\n"
            f"*(Connect `AGENTROUTER_API_KEY` in backend .env to activate Claude 3.5 Sonnet)*"
        )
...
**Topic:** {topic}  
**Status:** Autonomous Intelligence Scan Complete  
...
---
> ⚡ *Powered by IntelPulse Engine. Set `AGENTROUTER_API_KEY` in environment variables for Claude 3.5 Sonnet deep synthesis.*
"""
